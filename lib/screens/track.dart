...
Future fetchPost(String cnote) async {
  final response = await http.get(
      'https://live.skyking.co/api/Track/ConsignmentMTrack_Mobile?true&cnote=' +
          cnote +
          '&key=skm&Email=skm%40flyking.co.in',
...
Future fetchInfo(String id) async {
  final response = await http.get(
      'https://live.skyking.co/FlykingTransaction.asmx/GetBranchDetailsForTrack?ID=' +
          id,
      headers: {"Accept": "application/json"});
  //print(response.body);
  return response.body;
...
Future podLink(String id) async {
  final response = await http.get(
      'https://live.skyking.co/api/values/TrachPdfDisplay?vchDRSNo=' + id,
      headers: {"Accept": "application/json"});
...
class _TrackPageState extends State<TrackPage> {
                  ...
                  child: Column(
                      ...
                      children: <Widget>[
                        ...
                        CupertinoButton(
                          ...
                          onPressed: () => launch(
                              "tel:" + snapshot.data.contactNo.split(" ")[0]),
                        ),
                        CupertinoButton(
                          color: Colors.blue[100],
                          child: Row(
                          ...
                          onPressed: () => launch(
                              "tel:" + snapshot.data.contactNo.split(" ")[1]),
                        ),
                        CupertinoButton(
  ...
  List<Step> getSteps(BuildContext context, AsyncSnapshot snapshot) {
    ...
    for (int i = 0; i < snapshot.data.length; i++) {
      Step s = Step(
          title: Container(
              child: Text(snapshot.data[i].status),
          ...
          content: Container(
              child: Column(
            children: <Widget>[
              Text("Location: " + snapshot.data[i].location + "\n",
                ...
                onPressed: () {
                  showDialog(
                      ...
                      child: snapshot.data[i].branchId == "0"
                          ? AlertDialog(
                              title: Text("Conact Info"),
                              content: Text("Info Not Available"),
                            )
      ...
      body: FutureBuilder<List<Checkpoint>>(
          ...
          builder: (context, snapshot) {
            if (snapshot.hasData) {
              ...
              if (len == 0) {
                return Center(
                    child: Image(image: AssetImage("assets/404.jpg")));
              } else {
                if (s == "1") {
                  return Column(
                    children: <Widget>[
                        ...
                        child: Row(
                            ...
                            children: <Widget>[
                              ...
                              CupertinoButton(
                                ...
                                color: Colors.blue[100],
                                onPressed: () =>
                                    showpdf(snapshot.data[len - 1].refNo),
                              ),
                            ]),
                      ),
                      Stepper(
